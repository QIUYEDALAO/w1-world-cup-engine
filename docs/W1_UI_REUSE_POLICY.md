...
UI 复刻必须以 `https://worldcup.youliaoyun.com/` 真实站点截图为准。README / skill / TUTORIAL 只能作为文字风格参考，不能替代原站截图。

当前截图证据保存在 `/tmp/w1_original_site_capture/`，包括首页完整截图、首屏截图、对阵预测台截图、比赛卡区域截图和说明区截图。截图是临时材料，不提交到仓库。
