...
- 你不是决策器，不能决定推不推、不能改等级、不能改方向、不能覆盖 policy_result。
- Dashboard 主展示由本地 decision_card 生成；你的 recommendation_text 只是解释来源和 fallback，不是主决策结构。
...
- read.recommendation_text 是默认展示正文,必须包含 headline_cn、grade_cn、core_judgement_cn、reason_bullets_cn、score_recommendation_cn、ou_aux_cn、live_invalidation_cn。
- recommendation_text 必须先给结论,再解释为什么,再写什么时候失效;不要把默认区域写成数据格子。
- 若 recommendation_grade 为 A/B+/B, headline_cn 必须明确方向,如“AI亚盘推荐：巴拉圭 +0.5”;core_judgement_cn 必须解释为什么不是另一侧、为什么该盘口有保护、W1覆盖率与市场隐含差异。
- 若 PASS/观察, headline_cn 写“AI亚盘结论：PASS / 观察”;reason_bullets_cn 必须写盘口/覆盖率/edge/数据不足原因;live_invalidation_cn 写重新评估条件。
...
STYLE_MODES = {"conservative", "balanced", "aggressive_script"}
STYLE_PROMPTS = {
    "conservative": """
当前 style_mode=conservative。只给证据边界、PASS/观察、宽比分区间；不展开高比分尾部剧本，不夸大穿盘路径。
""",
    "balanced": """
当前 style_mode=balanced。给常规亚盘路径、大小球辅助、比分路径和 1-2 个风险；不夸大高方差，不把欧盘放回主轴。
""",
    "aggressive_script": """
当前 style_mode=aggressive_script。必须输出 script_layers.tail_script_cn，允许讨论穿盘路径、防线崩盘、早球后空间打开、大比分分支；但必须写触发条件和失效条件。若 data_readiness=低，禁止 A/B+、禁止 4-1/5-2、禁止强穿盘剧本。
""",
}


def get_system_prompt(style_mode: str) -> str:
    mode = style_mode if style_mode in STYLE_MODES else "balanced"
    return SYSTEM_PROMPT + "\n" + STYLE_PROMPTS[mode] + "\n输出还必须包含顶层 schema_version=scout_ah_recommendation_v2、style_mode、安全标签 safety_label，以及 read.script_layers{base_script_cn,tail_script_cn,reverse_script_cn,market_script_cn}。旧字段 regular_script_cn/high_variance_tail_script_cn/reverse_risks_cn/market_expert_script_cn 必须同步保留。"
...
def visible_text_values(read: dict[str, Any]) -> list[str]:
    ...
    if isinstance(layers, dict):
        out.extend(str(layers.get(key) or "") for key in SCRIPT_LAYER_KEYS)
...
def _score_risk_from_path(path: str) -> str:
    scores = re.findall(r"([0-9]+-[0-9]+)", str(path or ""))
    return scores[2] if len(scores) >= 3 else (scores[-1] if scores else "")


def normalize_recommendation_text(call: dict[str, Any], bundle: dict[str, Any] | None = None) -> None:
    read = call.get("read") if isinstance(call.get("read"), dict) else None
    if not read:
        return
    ah = read.get("asian_handicap_card") if isinstance(read.get("asian_handicap_card"), dict) else {}
    rec = read.get("recommendation_card") if isinstance(read.get("recommendation_card"), dict) else {}
    text = read.get("recommendation_text") if isinstance(read.get("recommendation_text"), dict) else {}
    home = str((bundle or {}).get("home") or "主队")
    away = str((bundle or {}).get("away") or "客队")
    grade = str(ah.get("recommendation_grade") or "PASS")
    confidence = str(ah.get("ah_confidence_cn") or rec.get("confidence_cn") or "低")
    pick = sanitize_visible_text(ah.get("main_ah_pick_cn") or rec.get("ah_pick_cn") or "亚盘推荐：PASS / 观察")
    current = sanitize_visible_text(ah.get("current_handicap_cn") or "")
    market = sanitize_visible_text(ah.get("market_consensus_cn") or rec.get("one_x_two_cn") or "欧盘仅作参考。")
    water = sanitize_visible_text(ah.get("water_movement_cn") or "水位变化暂无新增确认")
    movement = sanitize_visible_text(ah.get("line_movement_cn") or "盘口变化暂无新增确认")
    score_main = sanitize_visible_text(ah.get("score_main_cn") or "")
    score_backup = sanitize_visible_text(ah.get("score_backup_cn") or "")
    risk_score = _score_risk_from_path(str(ah.get("score_path_cn") or rec.get("score_picks_cn") or ""))
    cover_model = _pct1(ah.get("cover_probability_model"))
    cover_market = _pct1(ah.get("cover_probability_market"))
    edge = _pct1(ah.get("cover_edge"))
    grade_text = f"{grade}｜信心：{confidence}"
    is_pass = grade in {"PASS", "C/观察"} or "PASS" in pick or "观察" in pick
    if is_pass:
        headline = "AI亚盘结论：PASS / 观察"
        core = sanitize_visible_text(
            ah.get("pass_reason_cn")
            or "当前盘口、W1覆盖率或数据就绪度不足，暂不形成亚盘主推。"
        )
        reasons = [
            sanitize_visible_text(ah.get("pass_reason_cn") or "AH盘口、W1覆盖率或覆盖差不足。"),
            "当前数据不足以支撑主推，只保留观察。",
            f"{movement}；{water}，需要临场再确认。",
        ]
        invalid = [
            "若盘口回到可识别方向且水位稳定，可以重新评估。",
            "若W1覆盖率与市场隐含差扩大，再考虑升级。",
            "若首发关键点确认且无新增伤停，再复核方向。",
        ]
    else:
        headline = f"AI亚盘推荐：{pick}"
        side_text = "受让保护" if "受让" in str(ah.get("ah_side_cn") or "") or "+" in pick else "让球覆盖"
        core = (
            f"{home}略占优，但当前盘口没有给到主队足够穿盘空间。"
            f"{pick}有{side_text}，W1覆盖率 {cover_model}，市场隐含 {cover_market}，模型侧多出约 {edge}，因此主线更偏向{pick}。"
        )
        reasons = [
            f"{pick}覆盖平局或窄比分路径，容错高于单边穿盘方向。",
            f"{market}，平局权重不低，支持受让保护或浅盘谨慎。",
            f"比分路径集中在 {score_main or '主比分待生成'} / {score_backup or '备选待生成'}，不是大胜穿盘结构。",
            f"{movement}，{water}，当前盘口保护仍需临场维持。",
        ]
        invalid = [
            f"{current or pick} 退盘或水位明显反向，原方向失效并降级观察。",
            f"{home} 20分钟内早球，比赛进入主队压制或开放节奏，小球/受让路径失效。",
            f"{away} 首发防线或后腰关键点缺席，受让保护失效并降权。",
            "临场升到更深主让且低水，原受让方向降级观察。",
        ]
    score_text = f"主比分：{score_main or '比分待生成'}；备选：{score_backup or '比分待生成'}"
    if risk_score:
        score_text += f"；风险：{risk_score}"
    out = {
        "headline_cn": text.get("headline_cn") or headline,
        "grade_cn": text.get("grade_cn") or grade_text,
        "core_judgement_cn": text.get("core_judgement_cn") or core,
        "reason_bullets_cn": text.get("reason_bullets_cn") if isinstance(text.get("reason_bullets_cn"), list) else reasons,
        "score_recommendation_cn": text.get("score_recommendation_cn") or score_text,
        "ou_aux_cn": text.get("ou_aux_cn") or sanitize_visible_text(ah.get("ou_pick_cn") or rec.get("ou_pick_cn") or "大小球辅助暂不展开。"),
        "live_invalidation_cn": text.get("live_invalidation_cn") if isinstance(text.get("live_invalidation_cn"), list) else invalid,
    }
    out["headline_cn"] = _short_text(sanitize_visible_text(out["headline_cn"]), 56)
    out["grade_cn"] = _short_text(sanitize_visible_text(out["grade_cn"]), 32)
    out["core_judgement_cn"] = _short_text(sanitize_visible_text(out["core_judgement_cn"]), 160)
    out["score_recommendation_cn"] = _short_text(sanitize_visible_text(out["score_recommendation_cn"]), 96)
    out["ou_aux_cn"] = _short_text(sanitize_visible_text(out["ou_aux_cn"]), 96)
    out["reason_bullets_cn"] = [_short_text(sanitize_visible_text(item), 100) for item in out["reason_bullets_cn"] if str(item or "").strip()][:4]
    out["live_invalidation_cn"] = [_short_text(sanitize_visible_text(item), 100) for item in out["live_invalidation_cn"] if str(item or "").strip()][:4]
    while len(out["reason_bullets_cn"]) < 3:
        out["reason_bullets_cn"].append("证据不足，保持观察，不升级为更强结论。")
    while len(out["live_invalidation_cn"]) < 3:
        out["live_invalidation_cn"].append("若临场关键数据反向，当前方向降级观察。")
    read["recommendation_text"] = out
...
def sync_script_layers(read: dict[str, Any]) -> None:
    layers = read.get("script_layers") if isinstance(read.get("script_layers"), dict) else {}
    reverse = read.get("reverse_risks_cn")
    if isinstance(reverse, list):
        reverse_text = "；".join(str(x) for x in reverse if str(x or "").strip())
    else:
        reverse_text = str(reverse or "")
    # Canonical old fields win when both representations exist; this prevents
    # stale runtime script_layers from reintroducing old missing-market text.
    layers["base_script_cn"] = read.get("regular_script_cn") or layers.get("base_script_cn") or ""
    layers["tail_script_cn"] = read.get("high_variance_tail_script_cn") or layers.get("tail_script_cn") or ""
    layers["reverse_script_cn"] = reverse_text or layers.get("reverse_script_cn") or ""
    layers["market_script_cn"] = read.get("market_expert_script_cn") or layers.get("market_script_cn") or ""
    for key in SCRIPT_LAYER_KEYS:
        layers[key] = sanitize_visible_text(layers.get(key), market_script=(key == "market_script_cn"))
    read["script_layers"] = layers
    read["regular_script_cn"] = layers["base_script_cn"] or str(read.get("regular_script_cn") or "")
    read["high_variance_tail_script_cn"] = layers["tail_script_cn"] or str(read.get("high_variance_tail_script_cn") or "")
    if layers["reverse_script_cn"]:
        read["reverse_risks_cn"] = [layers["reverse_script_cn"]]
    read["market_expert_script_cn"] = layers["market_script_cn"] or str(read.get("market_expert_script_cn") or "")
...
def complete_script_guards(call: dict[str, Any]) -> None:
    ...
    visible = "\n".join(visible_text_values(read))
    has_market_line = bool(read.get("_market_has_lines"))
    if has_market_line and ("盘口数据缺失" in market_script or MARKET_MISSING_TEXT in visible or "市场赔率数据缺失" in visible):
        market_script = "若临场让球盘口退盘或水位反向，当前亚盘方向降权；大小球触发主要看早球与转换节奏。"
    elif ("盘口数据缺失" in market_script or MARKET_MISSING_TEXT in visible or "市场赔率数据缺失" in visible) and not has_market_line:
...
def chat_completion(cfg: dict[str, str], prompt: str, max_tokens: int, json_mode: bool = True, style_mode: str = "balanced") -> str:
    body_obj = {
        "model": cfg["model"],
        "temperature": 0.3,
        "max_tokens": max_tokens,
        "messages": [
            {"role": "system", "content": get_system_prompt(style_mode)},
...
def harden_call(call: dict[str, Any], fixture_id: str, bundle: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if isinstance(call.get("read"), dict):
        read = call["read"]
        if bundle_has_market_lines(bundle or {}):
            read["_market_has_lines"] = True
        ...
        for attempt in range(args.retries + 1):
            try:
                text = chat_completion(
                    cfg,
                    user_prompt(bundle, track, lessons, args.style_mode, validator_errors),
                    args.max_tokens,
...
def main() -> int:
    ...
    parser.add_argument("--style-mode", default=os.environ.get("W1_SCOUT_STYLE_MODE", "balanced"), choices=sorted(STYLE_MODES), help="Scout script tone: conservative / balanced / aggressive_script.")
    ...
    if args.dry_run:
        ...
        print(f"scout analyst dry-run: provider={args.provider}, selected={len(selected)}, model={model}, style_mode={args.style_mode}, read_output={CALLS_P.relative_to(ROOT)}")
        return 0
    if not selected:
