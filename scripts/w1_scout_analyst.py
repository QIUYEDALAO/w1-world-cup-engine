...
- read.evidence 是给 checker/专家审计用的结构化证据数组,至少 2 条；每条必须含 claim/source/fields/availability/weight。
...
- evidence_chain_cn 是给人看的中文证据链,要逐条引用可见数据来源:市场读数/形态/xG/阵容/伤停/排名/休息/环境;缺失也要写成证据,不得补造。
- 常规剧本写比赛最自然展开路径;尾部高方差剧本写早球、红牌、转换混乱、定位球、门将失误等低频但会改变节奏的路径。
- regular_script_cn 和 high_variance_tail_script_cn 都必须显式引用至少一个 evidence.claim 原文或 evidence.source,证明剧本不是空想。
...
- 专家盘口剧本只能解释让球/大小球/水位/早盘/临场/盘口样本,不得写成行动建议；盘口数据缺失时要明确写“盘口数据缺失/无法展开盘口剧本/不展开盘口剧本”,不要硬编盘口内容。
