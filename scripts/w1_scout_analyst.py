...
- 第一优先级是输出 read.recommendation_card：这是 dashboard 默认展示的 AI 推荐卡，必须短句、直接、可执行阅读；专家解释放在后面的证据链和剧本字段里。
...
- recommendation_card.score_picks_cn 使用“首选/次选/风险”短句；如果比分证据不足，写“比分候选降级观察：低比分小胜或平局分支，需结合临场数据。”
- recommendation_card.ou_pick_cn 使用“大/小2.5 倾向｜信心：高/中/低｜失效：...”；大小球盘口缺失时必须写“大小球盘口缺失，暂不输出大小球推荐。”
- recommendation_card.ah_pick_cn 使用“主队 -0.5 / 客队 +1 倾向｜信心：高/中/低｜理由：...”；让球盘口缺失时必须写“让球盘口缺失，暂不输出让球推荐。”
...
def market_prompt_line(market: dict[str, Any]) -> str:
    ...
    if all(isinstance(v, (int, float)) for v in vals):
        ...
        lines.append(f"市场胜平负倾向：主胜约 {home:.1%}，平局约 {draw:.1%}，客胜约 {away:.1%}。只作市场语境，不代表独立优势。")
...
def sanitize_visible_text(value: Any, *, score_band: bool = False, market_script: bool = False) -> str:
    ...
    text = re.sub(r"\b(?:数据项|来源|可用度|证据力度)\s*[:：=]", "证据：", text)
...
def normalize_recommendation_card(call: dict[str, Any]) -> None:
    ...
    card.setdefault("one_x_two_cn", "1X2 市场读数缺失，暂不展开胜平负推荐。")
    card.setdefault("score_picks_cn", "比分候选降级观察：低比分小胜或平局分支，需结合临场数据。")
    if has_market_line:
        card.setdefault("ou_pick_cn", "大小球倾向需结合盘口样本；信心：中；失效：早球或红牌打开节奏。")
        card.setdefault("ah_pick_cn", "让球倾向需结合盘口样本；信心：中；理由：只作受让保护与覆盖路径讨论。")
        card.setdefault("data_status_cn", "市场赔率可用 / 数据部分缺失")
    else:
        card.setdefault("ou_pick_cn", "大小球盘口缺失，暂不输出大小球推荐。")
        card.setdefault("ah_pick_cn", "让球盘口缺失，暂不输出让球推荐。")
        card.setdefault("data_status_cn", "市场赔率缺失 / 数据部分缺失")
    card.setdefault("main_recommendation_cn", "数据不足，推荐降级为观察；只保留主线剧本阅读。")
    card.setdefault("risk_cn", "若早球、红牌或阵容信息反转，当前主线需降权。")
...
def apply_visible_quality_guard(call: dict[str, Any]) -> None:
    read = call.get("read") if isinstance(call.get("read"), dict) else None
    if not read:
        return
    for key in VISIBLE_TEXT_KEYS:
    ...
    normalize_recommendation_card(call)
    if "xG样本不足" in "\n".join(visible_text_values(read)):
...
def harden_call(call: dict[str, Any], fixture_id: str) -> dict[str, Any]:
    ...
    if isinstance(call.get("read"), dict):
        ...
        apply_visible_quality_guard(call)
    return call
    ...
    for bundle in selected_bundles(fixtures, args.limit):
        ...
        for attempt in range(args.retries + 1):
            try:
                ...
                candidate = harden_call(candidate, fixture_id)
