...
Turns pre-match scout bundles into research calls with an Anthropic model.
The model is never trusted directly: each call must pass
check_w1_scout.validate_call before it is written to the gitignored
state/w1_scout_calls.json store.

No ANTHROPIC_API_KEY means no output is written. That keeps the cold-start path
honest and avoids fabricating analyst calls.
"""
...
def anthropic_message(api_key: str, model: str, prompt: str, max_tokens: int) -> str:
    body = json.dumps(
        {
            "model": model,
...
def build_calls(args: argparse.Namespace) -> tuple[list[dict[str, Any]], list[tuple[str, str]]]:
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        print("FAIL: ANTHROPIC_API_KEY is not configured; no scout calls written.", file=sys.stderr)
        raise SystemExit(2)
...
def write_calls(calls: list[dict[str, Any]], model: str) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate W1_SCOUT analyst calls through Anthropic, gated by check_w1_scout.")
    ...
    parser.add_argument("--model", default=os.environ.get("W1_SCOUT_ANTHROPIC_MODEL", DEFAULT_MODEL))
    parser.add_argument("--max-tokens", type=int, default=900)
    parser.add_argument("--retries", type=int, default=1, help="Validation retry count per fixture.")
    parser.add_argument("--dry-run", action="store_true", help="Validate setup and selected bundle count without calling Anthropic.")
    args = parser.parse_args()

    selected = selected_bundles(set(args.fixture or []) or None, args.limit)
    if args.dry_run:
        print(f"scout analyst dry-run: selected={len(selected)}, model={args.model}, output={CALLS_P.relative_to(ROOT)}")
        return 0
    if not selected:
        print("No scout bundles selected; no scout calls written.")
        return 0

    calls, failed = build_calls(args)
    if failed:
        ...
        return 1
    write_calls(calls, args.model)
    print(f"scout analyst PASS: wrote {len(calls)} calls -> {CALLS_P.relative_to(ROOT)}")
    return 0
