...
- 必须写出数据证据链、常规剧本、尾部高方差剧本、反向风险、专家盘口剧本。
- 数据证据链要逐条引用可见数据来源:市场读数/形态/xG/阵容/伤停/排名/休息/环境;缺失也要写成证据,不得补造。
- 常规剧本写比赛最自然展开路径;尾部高方差剧本写早球、红牌、转换混乱、定位球、门将失误等低频但会改变节奏的路径。
- 反向风险写与主倾向相反的比赛路径;专家盘口剧本只能解释让球/大小球/水位/早盘/临场/盘口样本,不得写成行动建议。
...
def user_prompt(bundle: dict[str, Any], track: dict[str, Any], lessons: str, validator_errors: list[str] | None = None) -> str:
    ...
    if validator_errors:
        retry_note = (
            ...
            "read 必须含 evidence_chain_cn / regular_script_cn / high_variance_tail_script_cn / reverse_risks_cn / market_expert_script_cn。"
        )
    return (
