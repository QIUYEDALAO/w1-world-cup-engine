...
def stage_rank(policy: dict[str, Any], stage_id: str) -> int:
    return stage_rank_map(policy).get(str(stage_id), 0)
...
def due_stage_for_record(rec: dict[str, Any], now: datetime, policy: dict[str, Any], forced_stage: str | None = None) -> dict[str, Any] | None:
    ...
    for stage in stages:
        ...
        grace = max(0, int(stage.get("window_grace_minutes") or 0))
        due_end = min(trigger + timedelta(minutes=grace), kickoff)
        item = {
            "fixture_id": fid,
            "match": rec.get("match"),
            "kickoff_utc": iso(kickoff),
            "trigger_at_utc": iso(trigger),
            "due_start_utc": iso(trigger),
            "due_end_utc": iso(due_end),
            "stage": stage,
        }
        if trigger <= now < due_end:
            current.append(item)
        elif trigger <= now < kickoff:
            missed.append(item)
    candidates = current or missed
    if not candidates:
        return None
    rank = stage_rank_map(policy)
    # One fixture can produce only one stage per scheduler run. Prefer the stage closest
    # to kickoff, so a late restart does not backfill every earlier window.
    return sorted(
        candidates,
        key=lambda r: (rank.get(str(r["stage"].get("stage_id")), 0), parse_dt(r["trigger_at_utc"]) or now),
        reverse=True,
    )[0]
...
def due_queue(now: datetime, fixture_id: str | None, forced_stage: str | None) -> list[dict[str, Any]]:
    ...
    for rec in load_records():
        if fixture_id and str(rec.get("fixture_id")) != str(fixture_id):
            continue
        item = due_stage_for_record(rec, now, policy, forced_stage)
        if item:
            rows.append(item)
    rank = stage_rank_map(policy)
    rows.sort(key=lambda r: (-rank.get(str(r["stage"].get("stage_id")), 0), parse_dt(r["trigger_at_utc"]) or now, str(r["fixture_id"])))
    return rows
...
def annotate_stage(fixture_id: str, stage: dict[str, Any], kickoff_utc: str) -> bool:
    ...
    retained = [
        c for c in calls
        if not (
            str(c.get("fixture_id")) == str(fixture_id)
            and str(c.get("stage_id") or "") in {"", sid}
        )
    ]
    ...
    unstaged = [c for c in candidates if not c.get("stage_id")]
    call = dict((unstaged or candidates)[-1])
    call["stage_id"] = sid
...
def run_stage(item: dict[str, Any], dry_run: bool) -> dict[str, Any]:
    ...
    env.update({
        ...
        "W1_SCOUT_EMBED_CMD": "true",
        "W1_SCOUT_LOCK_CMD": "true",
    })
    timeout_seconds = int(env.get("W1_SCOUT_SCHEDULER_STAGE_TIMEOUT_SECONDS", "240"))
    try:
        proc = subprocess.run(["bash", str(CYCLE)], cwd=ROOT, env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout_seconds)
    except subprocess.TimeoutExpired:
        return {"fixture_id": fid, "stage_id": sid, "result": "timeout", "message_cn": "AI分析超时；将在下一轮重试"}
    ...
    return {"fixture_id": fid, "stage_id": sid, "result": "success", "message_cn": f"{stage.get('label_cn')} {sid} 已生成"}
...
def run_once(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        print(f"scheduler dry-run now={iso(now)} pending_total={len(queue)} processed_count={len(process_queue)} pending_remaining_count={len(remaining)} max_fixtures={max_fixtures}")
    ...
    payload = {
        ...
        "pending_remaining_count": len(remaining),
        "pending_remaining_preview": [{"fixture_id": r["fixture_id"], "stage_id": r["stage"].get("stage_id"), "match": r.get("match")} for r in remaining[:8]],
        "failed_fixtures": [str(r.get("fixture_id")) for r in failed_results if r.get("fixture_id")],
        "results": results,
        "redlines_cn": "kickoff 后不补写赛前 read；dashboard 仅展示 scheduler 产物。",
    }
    write_json(STATUS, payload)
    print(f"scheduler once: pending_total={len(queue)} processed={len(process_queue)} success={len(success)} failed={len(failed_results)} remaining={len(remaining)} result={overall} status={STATUS.relative_to(ROOT)}")
